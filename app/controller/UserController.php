...
class UserController extends AdminBase
{
    ...
    function index()
    {
        if (Comm::isAjax()) {
            ...
            $endUnitList = array_column(UnitModel::ins()->where([
                'end_flag' => 'Y'
    ...
    function edit()
    {
        ...
        $arr['name'] = I('name');
        $arr['role'] = $role;
        $arr['user_code'] = $userCode = I('user_code');
        $arr['unit_code'] = I('unit_code');
        $arr['idno'] = I('idno');
        $arr['certificate_no'] = I('certificate_no');
        $arr['title'] = I('title');
        $arr['sex'] = intval(I('sex')) == 1 ? 1 : 2;
        
        // 修改
        $userid = intval(I('user_id'));
        if ($userid) {
            $user = UserModel::ins()->where([
                'user_code' => $userCode,
                'user_id' => [
                    'neq' => $userid
                ]
            ...
            if ($user){
                $this->error("该登录号已被{{$user['name']}}使用");
            }
            UserModel::ins()->update($arr, $userid);
        } else {
            $this->responseValidate([
                'user_code:登录号' => [
                    'notExists:UserModel,user_code' => '此登录号已存在，不能进行此操作'
                ]
            ]);
            UserModel::ins()->insert($arr);
        }
        $this->success();
    }
